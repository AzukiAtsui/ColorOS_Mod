...
elif [ "$API" -lt "33" ]; then echo " Hello, Android 12 user. ❛‿˂̵✧"
elif [ "$API" -eq "33" ]; then echo " Hello, Android 12 user. (＾Ｕ＾)ノ~"
fi
...
baddark=$(cat $MODCONFIG/blacklist_dark)
...
FUN_blacklistMv(){
	for APKN in $blacklistAPKNs; do
		sed -i '/'$APKN'/d' $pfd && echo "* 已删除含有黑名单应用包名：$APKN 的行" >&2
	done
}
...
apknda() { for APKN in $APKNs; do sed -i -e '/'$APKN'$/d' -e '$a'$APKN $pfd && echo "已去重添加包名：$APKN 到$NM" >&2; done; }
# add path of $SRC a varible named "${2}List" to $MODPATH/service.sh
apknlu() { if [[ "$SRC" == "$1" ]]; then sed -i '1i'"${2}List=$damCM$pfdDir/${SRC##*/}" $MODPATH/service.sh; echo 1 >$MODSIGN/${1##*/}; fi; }

FUN_sdmtam(){
	for APKN in $APKNs; do darkAPKN="<p\ attr\=\"$APKN\"\/>"
		if [[ -z "$(grep "$darkAPKN" $pfd)" ]]; then
			sed -i '/<\/filter-conf>/i'"$darkAPKN" $pfd && echo "已新添加APP包名：$APKN 到$NM" >&2
		else
			echo "包名：$APKN 已在$NM" >&2
		fi
	done
	for APKN in $baddark; do darkAPKN="<p\ attr\=\"$APKN\"\/>"
		sed -i '/'"$darkAPKN"'/d' $pfd && echo "* 已删除含有黑名单应用包名：$APKN 的行" >&2
	done
	tplFUN ${src_sdmtam%/*}/open_app "暗色模式开启应用名单" apknda
	# tplFUN 会重置 $pfd 的值
	for APKN in $baddark; do sed -i '/'$APKN'$/d' $pfd && echo "✘ 已从暗色模式开启应用名单删除包名：$APKN" >&2; done
